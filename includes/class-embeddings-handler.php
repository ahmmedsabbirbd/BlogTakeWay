...
class AISK_Embeddings_Handler
{
    ...
    public function __construct()
    {
        ...
        add_action('woocommerce_before_delete_product', [$this, 'delete_content_embeddings']);
        add_action('woocommerce_before_delete_product_variation', [$this, 'delete_content_embeddings']);
    }
    ...
    private function get_unprocessed_items($offset, $batch_size)
    {
        ...
        if (false === $items) {
            ...
            if (!empty($this->settings['ai_config']['woocommerce_enabled'])) {
        ...
        try {
            ...
            if ($has_settings_changes) {
                ...
                $settings_exists = $wpdb->get_var($wpdb->prepare(
                    "SELECT COUNT(*) FROM {$embedding_table} WHERE content_type = %s AND content_id = %d",
                    'settings', 0
                ));
                if (!$settings_exists) {
                    $count += 1;
        ...
        if (!empty($attributes)) {
            ...
            foreach ($attributes as $attribute) {
                if ($attribute->is_taxonomy()) {
                    ...
                    $content .= "{$attribute->get_name()}: " . implode(', ', $terms) . "\n";
                } else {
    ...
    private function get_variation_content($variation, $parent_product)
    {
        ...
        $content .= 'Variation Type: ' . $variation->get_type() . "\n\n";
        ...
        if (!empty($attributes)) {
            ...
            foreach ($attributes as $attr_name => $attr_value) {
                ...
                $content .= "{$attr_label}: {$attr_value_label}\n";
            }
            $content .= "\n";
        }
        ...
        if ($variation->get_sku()) {
            $content .= 'SKU: ' . $variation->get_sku() . "\n\n";
        }

        if ($variation->get_manage_stock()) {
            $content .= 'Stock Status: ' . $variation->get_stock_status() . "\n";
            $content .= 'Stock Quantity: ' . $variation->get_stock_quantity() . "\n\n";
        }

        if ($variation->get_description()) {
            $content .= "Description:\n" . wp_strip_all_tags($variation->get_description()) . "\n\n";
        }
    ...
    {
        // If it's a product search and WooCommerce is disabled, return empty
        ...
        try {
            ...
            if ($intent_type === 'product_search') {
                // Filter to keep only product types if any exist
                $product_results = array_filter($limited_results, function ($item) {
                    return $item['content_type'] === 'product';
                });
