...
const ChatWidget = React.forwardRef((props, ref) => {
    ...
    useEffect(() => {
        ...
        if (isOpen && conversationId && isInitialized && !isSendingMessage.current && !isNewConversation) {
    ...
    // Load conversations when chat is opened or when starting a new conversation
    useEffect(() => {
        if (isOpen && isInitialized) {
            loadRecentConversations();
        }
    }, [isOpen, isInitialized, conversationId]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Add user message to state after ensuring conversation exists
            ...
            // Even on error, ensure the user message is shown
            setMessages(prevMessages => {
                // Check if the user message is already in the state
                const hasUserMessage = prevMessages.some(msg =>
                    msg.type === 'user' && msg.content === userMessage
                );

                return [
                    ...prevMessages,
                    // Add user message if it's not already there
                    ...(hasUserMessage ? [] : [newUserMessage]),
                    {
                        type: 'bot',
                        messageType: 'bot',
                            ...
                            <>
                                {messages.length === 0 || messages.length === 1 ? (
                                    ...
                                    <>
                                        ...
                                        {isLoading && (
                                            <div className="support-buddy-typing">
                                                <div className="support-buddy-typing-dot"></div>
                                                <div className="support-buddy-typing-dot"></div>
                                                <div className="support-buddy-typing-dot"></div>
                                            </div>
                                        )}
